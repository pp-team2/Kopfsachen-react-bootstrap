...
const Routes = () => {
    ...
    const routes = [{
            path: ["/", "/home"],
            component: < App sessionActive = { sessionActive }
            check = { checkSession }
    ...
    return ( <
        ...
        Switch > {
            wikii.filter(a => a.id !== '').map(wikiEntry => {
                ...
                return ( <
                    Route key = { wikiEntry.id }
                    path = { `/wiki/${wikiEntry.id.replace(/\s+/g, '')}` } >
...

        {
            routes.map((route, index) => {
                return ( <
                    ...
                    <
                    Header color = { route.color }
                    text = { route.text }
                    img = { route.img }
                    /> {
                        (route.requiresSession && !sessionActive) ? < Zugriff sessionActive = { sessionActive }
                        check = { checkSession }
                        expertView = { expertView }
                        setExpertView = { setExpertView }
                        /> : route.component
